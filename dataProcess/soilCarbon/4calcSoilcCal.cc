...
int main(int argc, char **argv) {
  ...
  ofstream fpo[2];
  fpo[0].open("/tmp/soilc.bin", ios::out | ios::binary);
  fpo[1].open("/tmp/soilc_ha.bin", ios::out | ios::binary);
  map<int, ab> total;
  int lines=933120000;
  while (fp[0].good() && lines > 0) {
    ...
    totalC *= f;
    total[val[0]].bm += totalC;
    b2f.f = totalC;
    if(fp[0].good()) {for(int i=0; i<4; ++i) {fpo[0].put(b2f.c[i]);}}
    double forestArea = (val[1]+val[2])/100. * val[5]/10000.;
    if(forestArea > 0.) {b2f.f = totalC / forestArea;
    } else {b2f.f = 0.;}
