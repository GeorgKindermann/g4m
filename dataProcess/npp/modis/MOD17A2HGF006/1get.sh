...
for i in $(curl -s $LINK/ | grep -Po '(?<=href=")[^"]*' | grep -P "^\d{4}\.\d{2}\.\d{2}/"); do #Go to all folders
    ...
    rm MOD17A2HGF.*.hdf
    python3 /usr/bin/gdal_merge.py -o psnNetMOD17A2HGF006_${i:0:10}Sin.tif -co "BIGTIFF=YES" -co "COMPRESS=ZSTD" -co "TILED=YES" -co "PREDICTOR=2"  MOD17A2HGF.*.tif
    #gdalwarp -t_srs EPSG:4326 -co "BIGTIFF=YES" -co "COMPRESS=ZSTD" -co "PREDICTOR=2" -co "TILED=YES" psnNetMOD17A3HGF006_${i:0:10}Sin.tif psnNetMOD17A3HGF006_${i:0:4}LatLon.tif
    rm MOD17A2HGF.*.tif
done
