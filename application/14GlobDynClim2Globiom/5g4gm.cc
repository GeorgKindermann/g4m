//time ./5g4gm /data/tmp/w_hadgem2.bin /data/tmp/g4m_w_hadgem2_pas1.csv >/data/tmp/g4m_w_hadgem2_Test.csv
 ...
 int main(int argc, char **argv) {
  ...
  while(IN.good()) {
    ...
    globiomOutput tmp;
    double tmpD;
    IN >> col >> row >> alticlass;
    for(int i=0; i<decades; ++i) {IN >> tmp.currentForestArea[i];}
    for(int i=0; i<decades; ++i) {IN >> tmp.currentForestAreaIrri[i];}
    for(int i=0; i<decades; ++i) {IN >> tmp.potentialForestArea[i];}
    IN >> tmp.primaryForest;
    union {
      unsigned short int i;
      unsigned char c;
    } cval;
    IN >> cval.i; tmp.shareDecidousBroadleaved = cval.c;
    IN >> cval.i; tmp.shareDecidousNeadleleaved = cval.c;
    IN >> cval.i; tmp.shareEvergreenBroadleaved = cval.c;
    IN >> cval.i; tmp.shareEvergreenNeadleleaved = cval.c;
    for(int decade=0; decade<decades; ++decade) {
      for(int i=0; i<numberOfMaiTypes; ++i) {
	IN >> tmp.maiSum[i][decade];}}
    for(int decade=0; decade<decades; ++decade) {
      for(int i=0; i<numberOfMaiTypes; ++i) {
	IN >> tmp.maiArea[i][decade];}}
    for(int i=0; i<decades; ++i) {IN >> tmp.maiSumBest[i];}
    for(int i=0; i<decades; ++i) {IN >> tmp.maiAreaBest[i];}
    IN >> tmp.cstemMan;
    IN >> tmp.cstemUman;
    IN >> tmp.slopet10;
    IN >> tmp.slope15;
    ...
    ss >> t1;
    while(ss >> t2) {
      tmp.countryLandarea[t1]=t2;
      ss >> t1;
    }
    grid[col][row][alticlass] = tmp;
  }
  ...
  for(int row=0; row<360; ++row) {
    for(int col=0; col<720; ++col) {
      //if(col==516 && row==113) {continue;} //IN DIESER ZELLE GIBTS NEN FEHLER!!
