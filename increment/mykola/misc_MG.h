...
//    ipol(const ipol<IDX, VAL> & g);    //MG: added to make it compatible with the previous version
    ...
    VAL v(const IDX);   //returns the value //MG: added to make it compatible with the previous version
  ...
  private:
//    std::map<IDX, VAL> aMap; //MG: changed to make it compatible with the previous version (moved to public)
...
//---------------------------------
  ...
  inline VAL ipol<IDX, VAL>::ip(const IDX &i, const IDX &i0, const IDX &i1, const VAL &v0, const VAL &v1) {
    ...
    if(i0 == i1) {return((v0 + v1)/2.);}      //MG: uncommented
