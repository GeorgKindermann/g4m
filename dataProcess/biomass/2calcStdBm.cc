...
int main(int argc, char **argv) {
  ...
  ofstream fpo[6];
  fpo[0].open("/data/tmp/baseBmHa.bin", ios::out | ios::binary);
  fpo[1].open("/data/tmp/baseBmHaManNoirri.bin", ios::out | ios::binary);
  fpo[2].open("/data/tmp/baseBmHaUmanNoirri.bin", ios::out | ios::binary);
  fpo[3].open("/data/tmp/baseBmHaManIrri.bin", ios::out | ios::binary);
  fpo[4].open("/data/tmp/baseBmHaUmanIrri.bin", ios::out | ios::binary);
  fpo[5].open("/data/tmp/baseBmMaxHa.bin", ios::out | ios::binary);
  map<int, ab> total;
  while (fp[0].good()) {
    union {
      float f;
	...
	double bm1 = 0.;
	bmMax += iTab.gAvgBm(iTab.gTopt(mai,1), mai) * shareIrri;
	for(int j=0; j<5; ++j) {
