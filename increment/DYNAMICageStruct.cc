...
namespace g4m {
  ...
  ageStruct::v ageStruct::deforest(double area, int type) {
	...
	for(int i=0; i<activeAge; ++i) {
	  double sdEt = it->gSd(i, maiLongTime, dat[i].bm, 2);
  ...
  double ageStruct::setRotPeriod(int rotPeriod) {
    ...
    else {targetRotationPeriod = it->gTopt(maiLongTime, 0);} //Unmanaged Rotation time
    updateHarvestArea();
  ...
  double ageStruct::setMaiLongTime(double amaiLongTime) {
    if(ageClasses < it->gTopt(amaiLongTime, 0)) {setAgeClasses(it->gTopt(amaiLongTime, 0));}
    maiLongTime = amaiLongTime;
    return(maiLongTime);
  }
  ...
  ageStruct::v ageStruct::aging() {
	...
	if(changingNpp) {
	  double maxBmt = it->gHbmt(i, maiLongTime);
	  double maxBm = it->gHbm(i, maiLongTime);
	  std::pair<double,double> tmpTypicalThin = it->gDbmt(i, maiLongTime, maxBmt * targetSd, targetSd);
	  double stableThin = (tmpTypicalThin.second * 3. + thin)/4.;
	  double stableDbm = dbm + thin - stableThin;
	  if((dat[i].bm + stableDbm) > maxBm*1.2) {
	    double maxDbm = maxBm*1.2 - dat[i].bm;
	    stableThin += stableDbm - maxDbm;
	    stableDbm = maxDbm;
	  }
	  dbm = stableDbm;
	  thin = stableThin;
	...
	if(changingNpp) {
	  double maxBm = it->gHbm(i, maiLongTime);
	  std::pair<double,double> tmpTypicalThin = it->gDbm(i, maiLongTime, maxBm, 1);
	  double stableDbm = (dbm + tmpTypicalThin.first)/2.;
	  if((dat[i].bm + stableDbm) > maxBm*1.2) {
	    stableDbm = maxBm*1.2 - dat[i].bm;
	  }
	  dbm = stableDbm;
	}
      }
      double id = it->gDi(i, mai, it->gSd(i, maiLongTime, dat[i].bm, 2));
  ...
  (int rotationPeriod, double area, double sd) {
    ...
    for(int i=0; i<rotationPeriod; ++i) {
      if(targetSd > 0.) {
	tmp.bm = it->gHbmt(i, maiLongTime) * targetSd;
	std::pair<double,double> tt = it->gDbmt(i, maiLongTime, tmp.bm, targetSd);
	dbm=tt.first; thin=tt.second;
	tmp.d =  it->gD(i, maiLongTime, targetSd);
	tmp.h =  it->gH(i, maiLongTime);
      }
      else {
	tmp.bm = it->gHbm(i, maiLongTime);
	std::pair<double,double> tt = it->gDbm(i, maiLongTime, tmp.bm, 1.);
	dbm=tt.first; thin=tt.second;
	tmp.d =  it->gD(i, maiLongTime, 999.);
	tmp.h =  it->gH(i, maiLongTime);
      }
      dat[i] = tmp;
    }
  ...
  (incrementTab *ait, ipol<double,double> *asws
   ...
   , double aminDbVn
   , double aminDbEn, double amai, double amaiLongTime
   , int arotationPeriod, double astockingDegree
   , double aarea, double aminRot, bool achangingNpp)
  {
    activeAge = 0;
    it = ait;
    sws = asws;
    ...
    mai = amai;
    maiLongTime = amaiLongTime;
    ageClasses = it->gTopt(maiLongTime, 0);
