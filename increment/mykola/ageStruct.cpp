//r15 by georg.kindermann on Mar 28, 2012   
...
namespace g4m {
  ...
  (double rotationPeriod, double aarea, double sd) {
	...
	if (ageClasses<2) ageClasses = 2; //MG: out of range problem
    if(dat.size() < ageClasses) {dat.resize(ageClasses);}
//    if(dat.size() < ageClasses+1) {dat.resize(ageClasses+1);} //MG
  ...
  std::pair<ageStruct::v, ageStruct::v> ageStruct::aging(double amai) {
    ...
    setRotationTime();
    setMinRot();
//std::cout<<"after setMinRot"<<std::endl;
...
//    int i = static_cast<int>(dat.size())-1; MG: out of range problem!
//    int i = 0;
//	if (dat.size()<2) {i = static_cast<int>(dat.size())-1;} //MG: out of range problem!
//	else {i = static_cast<int>(dat.size())-2;} // MG
    int i = static_cast<int>(dat.size())-2;// MG: out of range problem!
...
//    for(int i = static_cast<int>(dat.size())-1; i>-1; --i) { //MG: out of range problem
    for(int i = static_cast<int>(dat.size())-2; i>-1; --i) { //MG: out of range problem
      if(dat[i].area > 0.) {
        double sdNat;
 ...
 //Stocking degree to high or typical thinnigs are forced -> make thinning
        ...
        if(i < static_cast<int>(dat.size())-1) {//MG: out of range problem ????
          dat[i+1] = dat[i];
        } else {
          dat[i+1].d = (dat[i+1].d * dat[i+1].area + dat[i].d * dat[i].area) / (dat[i+1].area + dat[i].area);//MG: out of range problem
          dat[i+1].h = (dat[i+1].h * dat[i+1].area + dat[i].h * dat[i].area) / (dat[i+1].area + dat[i].area);//MG: out of range problem
          dat[i+1].bm = (dat[i+1].bm * dat[i+1].area + dat[i].bm * dat[i].area) / (dat[i+1].area + dat[i].area);//MG: out of range problem
          dat[i+1].area += dat[i].area;
        }
