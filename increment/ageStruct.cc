...
namespace g4m {
  ...
  double ageStruct::reforest(double aarea) {
    ...
    if(aarea > 0) {
      dat[0].area += aarea/2.;
      dat[1].bm = (dat[1].bm * dat[1].area + it->gBmSdTab(0.5*timeStep, mai, 1) * aarea/2.) / (dat[1].area + aarea/2.);
      dat[1].area += aarea/2.;
      area += aarea;
    }
 ...
 //Stocking degree to high or typical thinnigs are forced -> make thinning
  ...
  ageStruct::v ageStruct::calamity(ffipol<double> *share, int type) {
    v ret = {0., 0., 0., 0., 0.};
    std::vector<double> dbhBm(2,0);
    double S;
    for(size_t i=1; i<dat.size(); ++i) {
